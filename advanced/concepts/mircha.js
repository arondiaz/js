...
const alCuadrado = (array = undefined) => {
  if (array === undefined) return console.log("No ingresaste números");

  if (!(array instanceof Array))
    return console.log("El valor que ingresaste no es un arreglo");

  if (array.length === 0) return console.log("El array está vacío");

  let result = [];
  for (const iterator of array) {
    if (typeof iterator !== "number") {
      return console.log("Debe ingresar números");
    }
    result.push(iterator * iterator);
  }
...
const maxMin = (array) => {
  ...
  maxAndMin.push(Math.min(...array));
  maxAndMin.push(Math.max(...array));

  return console.log(maxAndMin);
};
...
console.log(parImparArr([1, 2, 3, 4, 5, 6, 7, 8, 9]));
